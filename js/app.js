    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".testimonials__slider")) new core(".testimonials__slider", {
                ...
                loop: true,
                navigation: {
                    prevEl: ".testimonials__arrow-left.swiper-button-prev",
                    nextEl: ".testimonials__arrow-right.swiper-button-next"
                },
                on: {}
            });
            if (document.querySelector(".body-product__image-preview")) {
...
***************************************************************************** */
        ...
        function windowLoad() {
            seriesElement = document.querySelector(".series");
            categoriesElements = document.querySelectorAll(".series__column");
            console.log(seriesElement);
            console.log(categoriesElements);
            if (seriesElement) loadProducts();
        }
        async function loadProducts(start = 0, limit = 9) {
            const productsFiles = "files/json/colections.json";
            const response = await fetch(productsFiles, {
                metod: "GET"
            });
            if (response.ok) {
            ...
            function setProducts(products, start, limit) {
                ...
                const productsArray = products.colections;
                productsArray.forEach(((product, index) => {
                    console.log(product);
                    if (index >= start && index < limit + start) {
                        let productTemplate = `\n      <article id="pid-${product.id}" class="series__items items-series">\n        <a href="${product.url}" class="items-series__body">\n          <div class="items-series__image-ibg_contain">\n            <img src="${product.image}" alt="image">\n          </div>\n          <div class="items-series__content">\n            <div class="items-series__value">${product.title}</div>\n            %OLDPRICE%\n            <div class="items-series__price">Rp ${product.price.value}</div>\n          </div>\n        </a>\n      </article>\t\t\n      `;
                        console.log(product.price.priceOld);
                        if (product.price.priceOld) {
                            const priceOld = `<div class="items-series__price-old">Rp ${product.price.priceOld}</div>`;
                            productTemplate = productTemplate.replace("%OLDPRICE%", priceOld);
                        } else productTemplate = productTemplate.replace("%OLDPRICE%", "");
                        console.log(productTemplate);
                        const category = product.category;
                        console.log(category);
                        let colectionsColumn;
                        if (1 === category) colectionsColumn = document.querySelector("#cat-01");
                        if (2 === category) colectionsColumn = document.querySelector("#cat-02");
                        if (3 === category) colectionsColumn = document.querySelector("#cat-03");
                        colectionsColumn.insertAdjacentHTML("beforeend", productTemplate);
                    }
                }));
            }
        }
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".items-series__border-button")) {
                const productsQuantity = document.querySelectorAll(".series__items").length;
                console.log(productsQuantity);
                loadProducts(productsQuantity);
                e.preventDefault();
            }
            if (targetElement.closest(".body-product__button")) {
                addToBasket();
                e.preventDefault();
            }
        }
        function addToBasket() {
            const speed = 1e3;
            const headerBag = document.querySelector(".action-header__bag");
            const headerBagQuantity = headerBag.querySelector("span");
            ...
            document.querySelector(".body-product__price-old") && document.querySelector(".body-product__price-old").innerHTML;
            document.querySelector(".body-product__price").innerHTML;
            ...
            productImageBlock.classList.add("image-fly");
            productImageBlock.append(productImageClone);
            document.body.append(productImageBlock);
            productImageBlock.style.cssText = `\n    position: fixed;\n    top: ${productImage.getBoundingClientRect().top}px;\n    left: ${productImage.getBoundingClientRect().left}px;\n    width: ${productImage.offsetWidth}px;\n    height: ${productImage.offsetHeight}px;\n  `;
            setTimeout((() => {
                productImageBlock.style.cssText = `\n    top: ${headerBag.getBoundingClientRect().top}px;\n    left: ${headerBag.getBoundingClientRect().left}px;\n    width: 0;\n    height: 0;\n    transition: all ${speed}ms;\n  `;
            }), 5);
            setTimeout((() => {
                headerBagQuantity.style.cssText = `\n      opacity: 1;\n    `;
                headerBagQuantity.innerHTML = +headerBagQuantity.innerHTML + +productQuantity;
                productImageBlock.remove();
            }), speed);
        }
        window["FLS"] = true;
        isWebp();
